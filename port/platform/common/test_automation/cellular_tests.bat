...
:build_platform_4
    ...
    echo %~n0: print version information end
    echo %~n0: building tests and then downloading them over %com_port%...
    rem The Nordic Makefile can only handle a single sub-directory name which
    rem must be off the directory that Makefile is located in, so need to be
    rem in the Makefile directory for building to work and we use a fixed name
    rem 'cos there's little point in doing anything else
...
:usage
    ...
    echo.
    echo Note that the installation of various tools and manipulating processes requires administrator privileges
    echo and so this batch file must be run as administrator.
    goto end
